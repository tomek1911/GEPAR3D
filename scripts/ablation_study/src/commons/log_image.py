...
class Logger():
    ...
    def __init__(self,
        ...
        self.binary_color_map = slicer_colorspace[:2]
        #default as multiclass
        self.listed_slicer_map = colors.ListedColormap(self.color_map, 'slicer_colors')
    ...
    def log_image(self, prediction: torch.tensor, label: torch.tensor, image: torch.tensor) -> np.array:
        ...
        if x.shape[0]!=x.shape[2]:
            x=self.pad_to_square(x,pad_value=0)
            y=self.pad_to_square(y,pad_value=0)
            z=self.pad_to_square(z,pad_value=0)
    ...
    def log_image_multitask(self, 
                            distance_map_pred, distance_map_gt,
                            seed_map_pred, seed_map_gt,
                            seg_map_pred, seg_map_gt,
                            input_image,
                            seg_map_multiclass_pred = None, seg_map_multiclass_gt = None) -> np.array:
        is_multiclass = False
        if seg_map_multiclass_pred is None:
            # 7 elements 
            input_data = [distance_map_pred, distance_map_gt, seed_map_pred, seed_map_gt, seg_map_pred, seg_map_gt, input_image]
        else:
            is_multiclass = True
            #9 elements
            input_data = [distance_map_pred, distance_map_gt, seed_map_pred, seed_map_gt, seg_map_pred, seg_map_gt, seg_map_multiclass_pred, seg_map_multiclass_gt, input_image]
        ...
        for idx, img in enumerate(input_data):
            ...
            h_sl = np.rot90(img[:, h, :])
            d_sl = img[:, :, d]
            if is_multiclass and (idx == 6 or idx == 7):
                slices.extend([self.color_map[w_sl.astype(int)], self.color_map[h_sl.astype(int)], self.color_map[d_sl.astype(int)]])
            else:
                slices.extend([w_sl, h_sl, d_sl])
        ...
        img_log = cv2.vconcat([cv2.hconcat([i if len(i.shape) == 3 else cv2.cvtColor(i, cv2.COLOR_GRAY2RGB) for i in row]) for row in column])
        return img_log
    ...
    def log_scene(self, volume: np.array, num_classes: int = -1, add_volume_outline=False, scene_size: int = 480, is_zoom: bool = False, val: Optional[Union[str, int]] = 2.0, view: int = 3) -> np.array:
        ...
        p.add_mesh(tresh_data, cmap=self.listed_slicer_map, scalars="values", clim=[-.5, num_classes + 0.5], 
    ...
    def log_3dscene_comp(self, volume: np.array, volume_gt: np.array, num_classes: int = -1, gt_num_classes: int = -1, scene_size: int = 480, camera_pos : list = [0,-1,0], second_row : bool = True) -> np.array:
        ...
        if self.log_counter >= 200:
            ...
            return np.zeros(shape=(scene_size, scene_size, 3), dtype=np.uint8)
        else:
            self.log_counter +=1
        ...
        if num_classes == 1:
            listed_slicer_map = colors.ListedColormap(self.binary_color_map, 'slicer_colors')
            listed_slicer_map_pred = listed_slicer_map
            listed_slicer_map_gt = listed_slicer_map
            pred_labels=1
            gt_labels =1
        else:
            pred_labels = num_classes

            #chose subset of color map
            if gt_num_classes == -1:
                gt_labels = pred_labels 
            else:
                gt_labels = gt_num_classes
                
            listed_slicer_map_pred = colors.ListedColormap(self.color_map[:pred_labels+1], 'slicer_colors')
            listed_slicer_map_gt = colors.ListedColormap(self.color_map[:gt_labels+1], 'slicer_colors')
        ...
        #PLOT SCENES
        # prediction
        if tresh_data.n_points > 0:
            pred = p.add_mesh(tresh_data, cmap=listed_slicer_map_pred, scalars="values", clim=[-0.5, pred_labels + 0.5], 
                            scalar_bar_args=sargs_pred, smooth_shading=False)
            ...
            # ground_truth
            if second_row:
                if tresh_data_gt.n_points > 0:
                    gt = p.add_mesh(tresh_data_gt, cmap=listed_slicer_map_gt, scalars="values", clim=[-0.5, gt_labels + 0.5], 
                                    scalar_bar_args=sargs_gt, smooth_shading=False)
                    p.camera_position= camera_pos
...
def log_angles(pred, gt, mask):
    #padding
    edge = max(pred.shape)
    pad_transform = SpatialPad((edge,)*3, method = 'symmetric', mode= 'constant', value = 0)
    pred = pad_transform(pred).numpy()
    gt = pad_transform(gt).numpy()
    mask = pad_transform(mask).float().numpy()
    
    s = pred.shape[2]//2

    x_pred = pred[1,:,:,s]*mask[0,:,:,s]
    y_pred = pred[2,:,:,s]*mask[0,:,:,s]
    z_pred = pred[0,:,s,:]*mask[0,:,s,:]
